...
<head>
    ...
    <title>{{ __('Scan a barcode') }}</title>
    @vite(['resources/css/app.css', 'resources/js/scanner.js'])
</head>
<body class="bg-gray-900 text-gray-100 min-h-screen">
<main class="max-w-md mx-auto p-4 space-y-4">
    <h1 class="text-xl font-semibold text-center">{{ __('Scan a barcode') }}</h1>
    ...
    <form id="entry-form" class="space-y-3 hidden">
        ...
        <p id="entry-stale-warning" class="text-amber-400 text-sm hidden">
            {{ __("This price hasn't been confirmed in a while — please double check it.") }}
        </p>
    ...
    <div>
        <h2 class="text-sm font-semibold text-gray-300 mb-2">{{ __('This trip') }} (<span id="trip-count">0</span> {{ __('items') }})</h2>
    ...
    <div>
        <label for="trip-discount" class="block text-sm mb-1">{{ __('Coupons / discount for this trip (£)') }}</label>
